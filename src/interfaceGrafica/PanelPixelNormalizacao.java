...
public class PanelPixelNormalizacao extends JPanel {
	...
	public PanelPixelNormalizacao() {
		...
		labelPixelNormalizacao.setFont(new Font("Century Gothic", Font.BOLD, 30));
		...
		JLabel labelCoordenadasNormalizadas = new JLabel("Coordenadas Normalizadas");
		labelCoordenadasNormalizadas.setForeground(Color.WHITE);
		labelCoordenadasNormalizadas.setFont(new Font("Century Gothic", Font.BOLD, 20));
...
planoCartesiano = new BufferedImage(601, 601, BufferedImage.TYPE_INT_RGB);
		...
		JPanel panelBufferedImage = new JPanel(){
		    ...
		    public void paintComponent(Graphics g) {
				...
				for(int i = 0; i < planoCartesiano.getHeight(); i++){
					planoCartesiano.setRGB(i, planoCartesiano.getHeight()/2, Color.BLACK.getRGB());
					planoCartesiano.setRGB(planoCartesiano.getWidth()/2, i, Color.BLACK.getRGB());
				}
		...
		panelBufferedImage.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent eventoDeMouse) {
				labelNDH.setText("ndh (x): "+Integer.toString(eventoDeMouse.getX() - 300));
				labelNDV.setText("ndv (y): "+Integer.toString(((eventoDeMouse.getY() - 300)) * -1));
				labelNDCX.setText("ndcx : "+Float.toString(normalizacao.calcularNDCX(eventoDeMouse.getX() - 300, -300, 300)));
