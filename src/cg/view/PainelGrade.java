...
public class PainelGrade extends JPanel {
	...
	public PainelGrade(int x_size, int y_size) {
		...
		addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				e.consume();
				img_buffered.setRGB(c.dcx(e.getPoint().x),
						(int) c.dcy(e.getY()), Color.BLUE.getRGB());
				System.out.println("\t"+e.getPoint().x);
				repaint();
			}
		});
		/**
		 * AQui, acontecerá um evento que pegará a posição em que o Mouse está e
		 * será impresso no JLabel.
		 * 
		 */
		...
		JLabel lblCoordenadasDoDispositivo = new JLabel(
	...
	public void desenharUmPixel() {
		...
		addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				e.consume();
				img_buffered.setRGB((int) c.dcx(e.getX()),
						(int) c.dcy(e.getY()), Color.BLUE.getRGB());
				repaint();
			}
		});
